...
//One 24LC512 EEPROMs on the bus
const uint32_t totalKBytes = 32;         //for read and write test functions
extEEPROM eep(kbits_512, 1, 64, 0x50);         //device size, number of devices, page size
...
void setup(void)
{
  ...
  eepStatus = eep.begin();//eep.twiClock100kHz);
  Serial.print("status = "); Serial.println(eepStatus);
