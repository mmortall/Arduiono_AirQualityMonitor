...
#define	ILI9341_CS_PIN		2			// <= /CS pin (chip-select, LOW to get attention of ST7735, HIGH and it ignores SPI bus)
#define	ILI9341_DC_PIN		4 		// <= DC pin (1=data or 0=command indicator line) also called RS
#define	ILI9341_RST_PIN		3			// <= RST pin (optional)
#define	ILI9341_MOSI_PIN	11			// <= MOSI pin (if not hardware SPI)
//#define	ILI9341_MISO_PIN	0			// <= MISO pin (if not hardware SPI)
#define	ILI9341_SCLK_PIN	13			// <= SCLK pin (if not hardware SPI)
// (with hardware SPI some pins dictated by AVR PIN function - as shown above)

#define  TFT_CS    2     // <= /CS pin (chip-select, LOW to get attention of ST7735, HIGH and it ignores SPI bus)
#define TFT_DC   4     // <= DC pin (1=data or 0=command indicator line) also called RS
#define TFT_RST   3     // <= RST pin (optional)
#define TFT_MOSI  11      // <= MOSI pin (if not hardware SPI)
//#define ILI9341_MISO_PIN  0     // <= MISO pin (if not hardware SPI)
#define TFT_SCLK  13      // <= SCLK pin (if not hardware SPI)
